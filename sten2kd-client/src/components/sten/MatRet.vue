...
export default {
  ...
  methods: {
    ...
    async query () {
      ...
      try {
        var rst = await this.sten.MaterialReturn(begDate, endDate, this.filter.store, this.filter.cust)
        this.data = rst
      ...
      // 明细
      try {
        var entry = await this.sten.MaterialReturnEntry()
